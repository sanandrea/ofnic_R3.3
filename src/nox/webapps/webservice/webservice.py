...
def badRequest(request, errmsg, otherInfo={}):
    ...
    d["error"] = errmsg
    request.write(simplejson.dumps(d, sort_keys = True, indent=4))
    request.finish()
    return server.NOT_DONE_YET
...
def overLimits(request, errmsg, limits):
    """Return an error indicating this request method exceeds limits imposed by server"""
    request.setResponseCode(413, "Request entity too large")
    request.setHeader("Content-Type", "application/json")
    request.setHeader("Retry-After", limits['retryAfter'])
    d = {}
    d["displayError"] = "The server can not perform this operation."
    d["error"] = errmsg
    d["limits"] = limits
...
class WSPathTreeNode:
    ...
    def __init__(self, parent, path_component):
        ...
        self._doc_header = """<html><head></head><body style="font-family: sans-serif; font-size:14pt;">
            <table style="margin-top:10px;" width="100%"><tbody>
            <tr><th>Method</th><th>Path</th><th>Description</th><th>Consumes</th><th>Produces</th></tr>"""
            
        self._doc_footer = """</tbody></table></body></html>"""
    ...
    def set_handler(self, request_method, handler, doc, module_doc):
        ...
        d = self._tw.fill(d)
        self._handlers[request_method] = (handler, d, module_doc)
    

    def wrap_doc_in_html_table_row(self,request_type, path, doc=None, reserved=None):
        d = "<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>**</td><td>application/json</td></tr>".format(request_type,path,doc)
        return d
    
    def wrap_module_doc_in_html_table_row(self,module_doc):
        d = """<tr><td colspan="5" style="border-bottom:1px solid black;"><br/><b>{0}</b><br></td></tr>""".format(module_doc)
        return d


    def interface_doc(self, base_path, depth):
        msg = []
        if depth == 0:
            msg.append(self._doc_header)
            
        p = base_path + self.path_str()
        for k in self._handlers:
            #msg.extend((k, " ", p, "<br/>"))
            doc = self._handlers[k][1]
            module_doc = self._handlers[k][2]
            #if doc != None:
                #msg.extend((doc, "<br/>"))
                #msg.append("<br/>")
            if module_doc != None:
                msg.extend(self.wrap_module_doc_in_html_table_row(module_doc))
                
            msg.extend(self.wrap_doc_in_html_table_row(k,p,doc))
        for c in self._children:
            msg.append(c.interface_doc(base_path, depth + 1))
            
        if depth == 0:
            msg.append(self._doc_footer)
        return "".join(msg)
    ...
    def handle(self, t):
        ...
        if s != None:
            ...
            try:
                h, d, UoR = self._handlers[t.request_method()]
            except KeyError, e:
...
class WSRequestHandler:
    ...
    def interface_doc(self, base_path):
        ...
        d = """\
This is a RESTful web interface to OFNIC network applications.  The applications
running on this OFNIC instance support the following requests.<br/><br/>"""

        return d + self._path_tree.interface_doc(base_path, 0)
...
class WSLimits():
    ...
    counters = {}
    timestamps = {}
    methods = {'GET':0, 'POST':1, 'DELETE':2, 'PUT':3}
    
    def __init__(self, name):
        self.timestampsGlobal = [datetime.now()]
        self._name = name
        self.loop = task.LoopingCall(self.cleanDictionary)
        self.loop.start(self.cleanTimer) # call every minute
        #reactor is already running no need to launch it again

    def __str__(self):
        return self._name
        
    def fromTime2Sec(self, timeDelta):
    ...
    def getLimits(self, request):
        ...
        index = self.methods[request.method]
        now = datetime.now(pytz.utc)
        
        ret['remaining_GET'] = max (0, self.userLimit[0] - self.counters[uid][0])
        ret['remaining_POST'] = max (0, self.userLimit[1] - self.counters[uid][1])
        ret['remaining_DEL'] = max (0, self.userLimit[2] - self.counters[uid][2])
        ret['remaining_PUT'] = max (0, self.userLimit[3] - self.counters[uid][3])
        sec = self.fromTime2Sec(self.timestamps[uid][index] - now)
        
        d = timedelta(seconds = max(0,(self.interval -sec)))
        ret['retryAfter'] = (now + d).strftime("%Y-%m-%dT%H:%M:%SZ%z")
        return ret

    def isInsideLimits(self, request):
        now = datetime.now(pytz.utc)
        index = self.methods[request.method]
        
        if not self.checkGlobalLimits(now):
            return False
        ...
        if (uid in self.timestamps) and (uid in self.counters):
            #uid is in list
            #timestamp
            ts = self.timestamps[uid][index]
            #counter
            c = self.counters[uid][index]

            if (self.fromTime2Sec(now - ts) > self.interval):
                #too much time passed from last request of this type
                #update timestamp and counter
                self.timestamps[uid][index] = now
                self.counters[uid][index] = 1
                return True
            else:
                #check limits now
                if (c < self.userLimit[index]):
            ...
            #uid is not in list
            self.timestamps[uid] = [now,now,now,now]
            
            self.counters[uid] = [0,0,0,0]
            self.counters[uid][index] = 1
            return True      
            
    def checkGlobalLimits(self, now):
        #maybe Andi misunderstood global limits.
        #for the moment do not register global counters
        #TODO check this
        return True
        td = now - self.timestampsGlobal[len(self.timestampsGlobal) - 1]
        if self.fromTime2Sec(td) > self.interval:
            #put only the current timestamp on the list
            self.timestampsGlobal = [now]
            return True
        else:
            #secDiff < self.interval:
            #find the number of timestamps into list
            for item in self.timestampsGlobal:
                itemDiff = now - item
                itemDiffSeconds = itemDiff.seconds + itemDiff.days * 24 * 3600
                if itemDiffSeconds > self.interval:
                    self.timestampsGlobal.remove(item)
                        
            #now all old timestamps have been deleted
            #number of requests in last interval of time is equal to the length of the list
            self.timestampsGlobal.append(now)
            if (len(self.timestampsGlobal) > self.globalLimit):
                return False
            else:
                return True
...
class WSRes(webauth.AuthResource):
    ...
    def _limits(self, request, arg):
        request.setResponseCode(200)
        request.setHeader("Content-Type", "application/json")
        d = self.wsLimits.getLimits(request)
        request.write(simplejson.dumps(d, indent=4))
        request.finish()
    ...
    def render(self, request):
        ...
        if ("access-control-request-method" in allHeaders): 
            request.setHeader('Access-Control-Allow-Origin',"*")
            request.setHeader('Access-Control-Allow-Methods',"POST, GET, DELETE, OPTIONS, PUT")
...
class webservice(Component):
    ...
    def install(self):
        ...
        for v in self._versions:
            self.webserver.install_resource("/netic.v%s" % v, self._versions[v])
