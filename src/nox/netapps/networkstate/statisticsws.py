...
from nox.netapps.networkstate.pylinkload import PyLinkLoad
...
class statisticsws(Component):
    ...
    def install(self):
        ...
        self.ll = self.resolve(PyLinkLoad)
    ...
    def _get_path_stats(self,request, arg):
        ...
        flowLoad,error = self.ll.get_flow_load(monID)
        if (error == 0):
    ...
    def _get_all_mon_ids(self,request,arg):
        a = {}
        a['MonitorIDs'] = []
        outString = self.ll.get_path_mon_ids()
    ...
    def _remove_flow_monitor(self, request, arg):
        a = {}
        monID = int(str(arg['{Mon_ID}']),16)
        ret = self.ll.remove_monitor_flow(monID)
    ...
    def getInterface(self):
        return str(statisticsws)


def getFactory():
    class Factory:
        def instance(self, ctxt):
            return statisticsws(ctxt)
