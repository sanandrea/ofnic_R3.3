...
Disposition
Authenticator::handle_host_auth_lldp(const Event& e)
{
    VLOG_DBG(lg, "Lldp host event received.");
    const Lldp_host_event& lhe = assert_cast<const Lldp_host_event&>(e);

    const DLEntry *dlentry = get_dladdr_entry(lhe.dladdr);
    const HostNetEntry *netid;


    netid = get_names_from_bindings(dlentry, get_nwaddr_entry(lhe.nwaddr));
    if (netid == NULL) {
        VLOG_DBG(lg, "Binding for host doesnt exist, using default.");
        netid = get_auth_netid();
    }

    if (lhe.action == Lldp_host_event::AUTHENTICATE) {

        Host_auth_event *ha = new Host_auth_event(lhe.datapath_id, lhe.port,
                                              lhe.dladdr, lhe.nwaddr,
                                              netid->entry->host->name,
                                              netid->entry->name,
                                              300,18000,
                                              Host_event::AUTO_AUTHENTICATION);
        /*UoR : for timeouts see authenticator_util.cc & discovery.py */
        
        if (!add_host(*ha))
            VLOG_DBG(lg, "Error in authenticating host");
    } else if (lhe.action == Lldp_host_event::DEAUTHENTICATE) {
        /* UoR: not supported up to now */
        uint32_t enabled = Host_auth_event::EF_DLADDR | Host_auth_event::EF_NWADDR;
        Host_auth_event *ha = new Host_auth_event(lhe.datapath_id, lhe.port,
                                              lhe.dladdr, lhe.nwaddr,
                                              netid->entry->host->name,
                                              netid->entry->name,
                                              enabled,
                                              Host_event::DEAUTHENTICATION_EVENT);
        remove_host_by_event(*ha);
    }
