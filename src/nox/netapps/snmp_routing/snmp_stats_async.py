...
class snmp_stats(Component):
    ...
    def start_poll_snmp_thread(self):  
        ...
        aCmdGen = cmdgen.AsynCommandGenerator()
        mibBuilder = aCmdGen.snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder
        ...
        for dp1, port1, dp2, port2 in self.discovery.adjacency_list: #here i access to the adjacency_list that is in discovery
            ...
            for host in self.switchesIP:
                ...
                callbackContext = (dp1,port1,dp2,port2,host)
                #errorIndication, errorStatus, errorIndex, varBindTable = aCmdGen.nextCmd(
                sendRequestHandle = aCmdGen.asyncNextCmd(
                            cmdgen.CommunityData('public'),
                            cmdgen.UdpTransportTarget((ip_to_str_reversed(host), 161)),
                            #cmdgen.MibVariable('SNMPv2-MIB', 'sysDescr', 0),
                            ((1,3,6,1,4,1,39178,100,1,1),),
                            #(1,3,6,1,2,1,31,1,1,1,1),
                            (self.cbSnmpPolling, callbackContext),
        ...
        #print self.ipToDatapathid
        #print "port id and name correlation"
        #print self.portidToPortname
        #print "***Start generation of graph for Dijkstra's algorithm***"

        self.buildNetGraph()
        #reschedule the statistics gathering  
        self.post_callback(self.SNMP_CHECK_PERIOD, lambda : snmp_stats.start_poll_snmp_thread(self))  
        if (counter > 0):
            aCmdGen.snmpEngine.transportDispatcher.runDispatcher()

    #see: http://src.gnu-darwin.org/ports/net-mgmt/py-snmp4/work/pysnmp-4.1.7a/
    #docs/pysnmp-tutorial.html#ASYNCH-ONELINER-APPS 
    def cbSnmpPolling(self,
                      sendRequestHandle, 
                      errorIndication, 
                      errorStatus, 
                      errorIndex, 
                      ...
                      cbCtx):
        #print "Callback"
        (dp1,port1,dp2,port2,host) = cbCtx
        if errorIndication:
        ...
        for varBindTableRow in varBindTable:
            for name, val in varBindTableRow:
                ...
                if statPrettified == "txbytes":
                    if (switchStats.has_key(idStat)):
                        print "Has key"
                    if (oldStats[idStat].has_key("ts_openflow")):
                        print "Has O Key"
                    if (((switchStats[idStat]["ts_openflow"] - oldStats[idStat]["ts_openflow"])/1000000)>0):
