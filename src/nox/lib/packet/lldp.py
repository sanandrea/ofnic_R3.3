...
class system_name_tlv:
    ...
    def parse(self):
        (typelen,) = struct.unpack("!H", self.arr[0:2])
        self.type = typelen >> 9
        assert(self.type == lldp.SYSTEM_NAME_TLV)
        self.strlen = typelen & 0x01ff
        assert(self.strlen <= 255)
        unpack_fmt = '!' + str(self.strlen) + 's'
        end_offset = 2 + self.strlen
        (self.system_name,) = struct.unpack(unpack_fmt, self.arr[2:end_offset])
    
    def __str__(self):
        return ''.join(['<Sys Name:',str(self.system_name),'>'])
        
    def tostring(self):
        typelen = 0
        typelen = self.type << 9
        typelen = typelen | (self.strlen & 0x01ff) 
        pack_str_fmt = '!HB'+str(self.strlen-1)+'s'
        return struct.pack(pack_str_fmt, typelen, self.system_name)

# Management Address TLV Format
#
# +------------+---------------------+-----------------------------+----------------+----------------------+----------+
# |  TLV type  | TLV information     |  Management address string  |  Mng Addr      | Management Address   | Rest of  |
# |            | string length       |  length 1 Byte              | subtype 1 Byte |  1 - 31 Bytes        |   TLV    |
# +------------+---------------------+-----------------------------+----------------+----------------------+----------+
#
# <--------- 2 Bytes ---------------> <------------------------------- 9 - 167 Bytes --------------------------------->
#
#Management Addres TLV
class management_address_tlv:
    def __init__(self, array = None):
        self.parser = False
        self.type   = 0
        self.strlen = 0
    ...
    def parse(self):
        (typelen,) = struct.unpack("!H", self.arr[0:2])
        self.type = typelen >> 9
        assert(self.type == lldp.MANAGEMENT_ADDR)
        self.strlen = typelen & 0x01ff
        assert(self.strlen <= 255)
        (mng_add_len,) = struct.unpack("!B",self.arr[2:3])
        
        #assert(mng_add_len <= 32)
        #assert(mng_add_len >= 2)
        #unpack_fmt = '!' + str(mng_add_len - 1) + 's'
        #end_offset = 4 + mng_add_len - 1

        #TODO As a very first step support only IPv4.
        assert(mng_add_len == 5)
        a = struct.unpack("!4B", self.arr[4:8])
        self.mng_addr = str(a[0])+'.'+str(a[1])+'.'+str(a[2])+'.'+str(a[3])
        self.mng_addr_int = ipstr_to_int(self.mng_addr)
            
    def __str__(self):
        return ''.join(['<Mng Addr:',str(self.mng_addr),'>'])
    ...
    def tostring(self):
        ...
        typelen = typelen | (self.strlen & 0x01ff) 
        pack_str_fmt = '!HB'+str(self.strlen-1)+'s'
        return struct.pack(pack_str_fmt, typelen, self.mng_addr)    
...
class lldp (packet_base):
    ...
    tlv_parsers[SYSTEM_NAME_TLV]= system_name_tlv
    tlv_parsers[MANAGEMENT_ADDR]= management_address_tlv
    ...
    def parse(self):
        ...
        #
        # First version: wrong because stucked on 'normal' packets
        #
        ##get System Name
        #ret = self.next_tlv(self.arr[pduhead:])
        #if ret == None:
        #    self.msg( '(lldp parse) error parsing System Name TLV' )
        #    return
        #pduhead += ret
        #if self.tlvs[len(self.tlvs)-1].type != lldp.SYSTEM_NAME_TLV:
        #    self.msg( '(lldp parse) error port System Name TLV missing' )
        #    return
        #
        ##get Management Address
        #ret = self.next_tlv(self.arr[pduhead:])
        #if ret == None:
        #    self.msg( '(lldp parse) error parsing Management Address TLV' )
        #    return
        #pduhead += ret
        #if self.tlvs[len(self.tlvs)-1].type != lldp.MANAGEMENT_ADDR:
        #    self.msg( '(lldp parse) error port Management Address TLV missing' )
        #    return
        ...
        while 1:
            ...
            if self.tlvs[len(self.tlvs)-1].type != lldp.SYSTEM_NAME_TLV:
                i = i +1
                #Then it is not the system name TLV

            if self.tlvs[len(self.tlvs)-1].type != lldp.MANAGEMENT_ADDR:
                i = i +1
                #Then it is not the management addresss TLV
